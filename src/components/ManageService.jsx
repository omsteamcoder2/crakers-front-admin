...
const ManageService = () => {
  ...
  const deleteService = async (slug) => {
    ...
    try {
      ...
      setServices(services.filter((service) => service.slug !== id))
    } catch (error) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-800 dark:text-gray-200">Manage Services</h1>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Total: {services.length} services</p>
        </div>
        <Link
          to="/addservices"
      ...
      {services.length === 0 && !error ? (
        <NoContent type={"services"} message="No services found. Add your first service!" />
      ) : (
        <>
          {/* Mobile view: card layout */}
          <div className="block sm:hidden space-y-3">
            {services.map((service) => (
              <div key={service.slug} className="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-100 dark:bg-gray-800 dark:border-gray-700">
                <div className="p-3">
                  <div className="flex items-center">
                    ...
                    <div>
                      <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{service.title}</div>
          ...
          <div className="hidden sm:block bg-white rounded-lg shadow overflow-hidden overflow-x-auto dark:bg-gray-800 dark:border dark:border-gray-700">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                {services.map((service) => (
                  <tr key={service.slug} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
